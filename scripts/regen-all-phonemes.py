"""
Regenerate ALL phoneme audio files using example words.
...
async def generate_word_audio(word, retries=3):
    """Generate TTS audio for a word with retry."""
    ...
    for attempt in range(retries):
        try:
            ...
            if os.path.exists(tmp_path) and os.path.getsize(tmp_path) > 500:
                ...
                print(f"    [retry {attempt+1}] Empty/Small file for '{word}'")
        except Exception as e:
            print(f"    [retry {attempt+1}] TTS error for '{word}': {e}")
...
async def main():
    ...
    print(f"Regenerating {len(keys)} phoneme audio files from example words...")
    print(f"Voice: {VOICE}\n")
